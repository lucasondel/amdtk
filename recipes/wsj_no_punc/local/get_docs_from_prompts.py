...
def dump_utts_into_docs(utt_ids, doc_d, utt_txt_d, utt_ph_d, out_word_dir,
    ...
    uniq_keys = []
    uniq_doc_ids = []
    utts_with_ids = []  # will be saved in data/
    utts_with_ids_ph = []  # will be saved in data/
    ...
    for doc_id in sorted(list(doc_d.keys())):
        ...
        for utt_id in utt_l2:
            ...
            try:
                ...
                uniq_utt[utt_txt_d[utt_id]] = 1
                utts_with_ids.append(utt_id + " " + utt_txt_d[utt_id])
                utts_with_ids_ph.append(utt_id + " " + utt_ph_d[utt_id])
    ...
    utts_with_ids = sorted(utts_with_ids)
    with open(pwd + "../data/" + base + "_utts_words.txt", "w") as fpw:
        fpw.write("\n".join(utts_with_ids) + "\n")

    utts_with_ids_ph = sorted(utts_with_ids_ph)
    with open(pwd + "../data/" + base + "_utts_phonemes.txt", "w") as fpw:
        fpw.write("\n".join(utts_with_ids_ph) + "\n")
...
def main():
    ...
    test_ids = read_simple_flist(pwd + '../data/test_93.keys')
    ...
    out_dir = os.path.realpath(pwd + "../../../../../../EVAL/topics/wsj_no_punc/") + "/"
    ...
    dump_utts_into_docs(sorted(train_ids), doc_d, utt_txt_d, utt_ph_d, train_out,
                        train_ph_out, txt_utt_d, pwd, "training")
    dump_utts_into_docs(sorted(test_ids), doc_d, utt_txt_d, utt_ph_d, test_out,
                        test_ph_out, txt_utt_d, pwd, "test")
